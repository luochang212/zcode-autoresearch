...
- **`plugin/mcp/server.ts`**：JSON-RPC stdio server，工具 init/run/log/export/clear；账本 `.auto/log.jsonl` 为 append-only 事实源。
...
CI（GitHub Actions，`.github/workflows/ci.yml`）在 push/PR 时跑 lint + fmt:check + typecheck + test，Node 24。
