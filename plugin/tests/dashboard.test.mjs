...
function connect(cwd) {
  ...
  const call = (method, params) => new Promise((res) => { const i = id++; pending.set(i, res); proc.stdin.write(JSON.stringify({ jsonrpc: "2.0", id: i, method, params }) + "\n"); });
  const tool = async (name, args) => { const m = await call("tools/call", { name, arguments: args }); return JSON.parse(m.result.content[0].text); };
...
test("SSE broadcasts jsonl-updated after log_experiment (live refresh)", async () => {
  ...
  await withServer(cwd, async (s) => {
    ...
    const { value } = await Promise.race([
      reader.read(),
      new Promise((_, rej) => setTimeout(() => rej(new Error("no SSE event within 3s")), 3000)),
    ]);
...
test("workingDir redirects the ledger, benchmark and git to the research dir", async () => {
  ...
  writeFileSync(join(cwd, ".auto", "config.json"), JSON.stringify({ workingDir: "work" }));
  writeFileSync(join(cwd, "work", ".auto", "measure.sh"), '#!/usr/bin/env bash\necho "METRIC time_ms=7"\n');

  await withServer(cwd, async (s) => {
    await s.tool("init_experiment", { name: "t", metric_name: "time_ms" });
    const run = await s.tool("run_experiment", { command: "bash .auto/measure.sh" });
    assert.equal(run.metric, 7);
    await s.tool("log_experiment", { status: "keep", metric: 7, description: "x" });
    assert.ok(existsSync(join(cwd, "work", ".auto", "log.jsonl")), "ledger in work dir");
    assert.ok(!existsSync(join(cwd, ".auto", "log.jsonl")), "no ledger in project dir");
