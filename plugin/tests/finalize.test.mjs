...
function setup() {
  ...
  execFileSync("git", ["checkout", "-qb", "autoresearch/exp"], { cwd, stdio: "ignore" });
  writeFileSync(join(cwd, "a.js"), "v2\n");
  execFileSync("git", ["add", "-A"], { cwd, stdio: "ignore" });
  execFileSync("git", ["commit", "-qm", "experiment: a"], { cwd, stdio: "ignore" });
  const c1 = git(cwd, ["rev-parse", "HEAD"]).trim();
  writeFileSync(join(cwd, "b.js"), "v2\n");
  execFileSync("git", ["add", "-A"], { cwd, stdio: "ignore" });
  execFileSync("git", ["commit", "-qm", "experiment: b"], { cwd, stdio: "ignore" });
...
test("finalize splits two non-overlapping kept commits into branches", () => {
  ...
  assert.equal(git(cwd, ["branch", "--show-current"]).trim(), "autoresearch/exp");
});
...
test("finalize rejects overlapping files across groups and rolls back", () => {
  ...
  execFileSync("git", ["add", "-A"], { cwd, stdio: "ignore" });
  execFileSync("git", ["commit", "-qm", "experiment: b2"], { cwd, stdio: "ignore" });
